...
commonApp.factory('dkHttp', ['$http', function ($http) {
    ...
    return {
        smartclient: function(_params){
            $http.get('libs/php/s-getsession.php').success(function(response){
                if(!response.state){
...
commonApp.directive('dkNav', ['$compile', 'dkHttp', function ($compile, dkHttp) {
    return {
        ...
        link: function (scope, iElement, iAttrs) {
            dkHttp.smartclient({url: 'libs/php/getmodel.php', success: function(response){
                    scope.navs = response;
                }
            })
    ...
    $httpProvider.interceptors.push(function ($rootScope, $q) {
        return {
            'request': function (config) {
                //请求执行
                if (!$('.mask')[0]) {
                    var _html = '<div class="mask"><i class="fa fa-spinner fa-spin"></i></div>';
                    $(_html).appendTo($('body'));
//                  console.log(1)
                }
